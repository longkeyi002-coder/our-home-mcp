...
class RelayProtocolTest {
    ...
    @Test
    fun emptyMcpNotificationResponseHasNoJsonContentType() {
        val id = RelayProtocol.parseRequest("""{"id":"notification","method":"mcp","path":"/mcp","body":"{}"}""")!!.id
        val root = Json.parseToJsonElement(RelayProtocol.response(id, 202, null)).jsonObject
        assertEquals(202, root.getValue("status").jsonPrimitive.content.toInt())
        assertEquals("", root.getValue("body").jsonPrimitive.content)
        assertFalse(root.containsKey("contentType"))
    }
}
