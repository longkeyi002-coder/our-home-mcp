...
|---|---|---|---|
| OH-30 / OH-32 | Earth / AI World / Fiction records cannot leak across factual queries | Add world/provenance storage + query isolation tests | TODO |
| OH-31 | Long-lived records preserve provenance/source/time | Existing observation source/confidence persistence tests; extend to new world schema | PARTIAL |
| OH-40 | Repeated events do not produce wake/message storms | Wake cooldown/dedupe tests | COVERED |
| OH-41 | User can correct/delete/revoke/pause | API/UI tests when controls exist | TODO |
| OH-42 | Sensitive sensing is not default | Android permissions/config acceptance | PARTIAL / MANUAL |
...
| OH-61 | Daily telemetry survives without control WSS | Android HTTPS upload integration test | PARTIAL / MANUAL |
...
| OH-64 | Runtime remains event-driven; no high-frequency LLM life loop | Wake/event scheduling tests + config review | PARTIAL |
| OH-65 | Model calls are bounded to cognition-worthy work | Resource-budget tests when budget layer exists | TODO |
| OH-67 | Provider/tool failures degrade gracefully | Existing FCM/Hermes retry tests; add AI World deterministic progression test | PARTIAL |
| OH-P1 | Real Android observation reaches persisted Life State | Real-device acceptance + server integration | PARTIAL / MANUAL |
