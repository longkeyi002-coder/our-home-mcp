...
| OH-47 | FCM notification opens the intended Our Home Chat/message destination and obeys preview policy | Notification payload/deep-link tests + foreground/background real-device acceptance; P5 explicitly stops at internal `maybe_share` and does not bypass this delivery boundary; P6.1 accepts feedback only after the proactive candidate is actually delivered; P6.2 reply interpretation creates no proactive message or notification | TODO / MANUAL |
...
| OH-67 | Provider/tool failures degrade gracefully | FCM/Hermes retry tests + Android Room retry + staged auth errors + production start scripts; AI World deterministic progression is isolated from provider failure; P4 reflection and identity maintenance are failure-isolated; P5 provider/contract/persistence/share-cap failures back off or fail closed without blocking Earth heartbeat/Wake/Care/Delivery; P6.1 malformed/undelivered/colliding feedback fails closed without delivery or external side effects; P6.2 provider/contract failure leaves reply unreviewed, applies bounded backoff and cannot block heartbeat or existing Care delivery | COVERED |
...
| OH-P6 | User feedback affects future strategy without direct overwrite | P4.5 protects Feedback→Preference→review→Soul bounds; P6.1 `relationship-feedback.test.ts` protects delivered-message binding, explicit-vs-ambiguous signal semantics, fixed strategy mappings, idempotency and no direct Soul/delivery/Android mutation; P6.2 `relationship-reply-review*.test.ts` protects observed/user-declared/inferred separation, strict proposal-only model output, no direct Preference/Soul writes, sparse budgets and Worker failure isolation; explicit user confirmation/correction/dismissal of inferred proposals, learned-strategy review/revocation and final P6 review remain pending | PARTIAL |
...
P6 remains partial: explicit user confirm/correct/dismiss lifecycle for inferred reply proposals, learned-strategy review/correction/revocation and final P6 Phase Review are still pending. Mere absence of interaction must not be automatically treated as dislike.
