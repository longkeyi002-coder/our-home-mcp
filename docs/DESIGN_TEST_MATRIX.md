...
|---|---|---|---|
| OH-30 / OH-32 | Earth / AI World / Fiction records cannot leak across factual queries | `world-boundary.test.ts` protects strict observation evidence; `record-boundary.test.ts` protects diary/action/relationship/activity boundaries, approval inheritance and world-filtered reads; `LONG_LIVED_RECORD_BOUNDARY_CLASSIFICATION.md` excludes Runtime control state from canonical memory; P4 preference/Soul corruption is rejected by generic AI World validation; P4.4 reflection input is AI World-only; P4.5 keeps Earth feedback separate from derived AI World evidence; P5 exploration/reflection/maybe-share remain AI World-only and cannot write Earth observations or delivery queues; P6.1 keeps relationship signals as explicit Earth records and uses P4.5 as the only Bridge to AI World preference evidence; P6.2 keeps reply interaction/text/model proposal as separate Earth `observed`/`user_declared`/`inferred` records and never crosses inferred proposals into P4.5; P6.3 requires a separate Earth/user-declared review before learning; P6.4 keeps Earth revocation, AI World revocation and archived AI World evidence distinct | COVERED |
| OH-31 | Long-lived semantic records preserve world/provenance/source/time | Observation schema + `record-boundary.test.ts` + `routine-boundary.test.ts`; diary/action/relationship/activity carry explicit boundaries and RoutineWindow is fixed to `EARTH/user_declared`; P4 continuity/preference/Soul records carry explicit AI World boundaries, timestamps and evidence traces; reflection output binds exact source review evidence; feedback Bridge keeps explicit Earth and AI World records; P5 Experience/Collection/share-intent records preserve exact topic/public-Web/reflection bases; P6.1 relationship signals bind an exact delivered proactive candidate, provenance and event time; P6.2 reply text is `EARTH/user_declared` while model interpretation is separately `EARTH/inferred`, both bound to the exact reply/message basis; P6.3 persists immutable inferred proposal plus separate user-declared review; P6.4 preserves original feedback/evidence plus separate revocation/audit records | COVERED |
| OH-40 | Repeated events do not produce wake/message storms | Wake cooldown/dedupe tests + `TelemetryPolicyTest` stable heartbeat bucket ID + Room unique dedupe key instrumentation coverage; P5 exploration memory and maybe-share intents use deterministic basis dedupe and a five-pending cap; P6.1 stable `signalKey` replay is idempotent and cannot reinforce the same product signal twice; P6.2 enforces one canonical reply text per reply and one proposal per deterministic reply/text basis; P6.3 stable `reviewKey` and one terminal review prevent duplicate learning; P6.4 stable `revocationKey` and one-feedback-at-most-once semantics prevent duplicate evidence removal/replacement | COVERED |
| OH-41 | User can correct/delete/revoke/pause and control per-App sensing | Configuration gate + explicit manual fallback + diagnostics copy without credential values; `PresencePrivacyRulesTest` protects per-App identity allow/hide semantics; P4.5 supports bounded corrective feedback; P6.1 records explicit delivered-message relationship feedback; P6.2 inferred reply proposals remain pending and cannot silently become learned preference; P6.3 protects explicit confirm/correct/dismiss; P6.4 protects bounded learned-evidence review/revoke/correct with audit preservation and active-basis recomputation. Product UI for learned-evidence review plus broader delete/pause controls remain pending | PARTIAL |
...
| OH-47 | FCM notification opens the intended Our Home Chat/message destination and obeys preview policy | Notification payload/deep-link tests + foreground/background real-device acceptance; P5 explicitly stops at internal `maybe_share` and does not bypass this delivery boundary; P6.1 accepts feedback only after the proactive candidate is actually delivered; P6.2 reply interpretation creates no proactive message or notification; P6.3/P6.4 review/revoke/correct paths create no notification/FCM work | TODO / MANUAL |
...
| OH-52 | Risk Level 2/3 actions require confirmation and remain independent from observation permissions | Action policy tests; P3 Level-0 AI World MCP is side-effect isolated; P5 exploration/share-intent contracts have no external-action surface; P6.1 feedback capture has no device/external action surface; P6.2 model output cannot send/notify/act or write Preference/Soul; P6.3/P6.4 deterministic review/revoke/correct contracts expose no Android/external action fields; Level 2/3 confirmation policy remains future work | TODO |
| OH-60 | Runtime Core is provider-neutral | `BrainAdapter` compile boundary + mock brain tests; P4.4 adds provider-neutral `AiWorldReflectionAdapter`; P5 adds provider-neutral `AiWorldExplorationAdapter` with concrete gateway adapter behind it; P6.2 adds provider-neutral `RelationshipReplyReviewAdapter`; add import-boundary guard | PARTIAL |
...
| OH-67 | Provider/tool failures degrade gracefully | FCM/Hermes retry tests + Android Room retry + staged auth errors + production start scripts; AI World deterministic progression is isolated from provider failure; P4 reflection and identity maintenance are failure-isolated; P5 provider/contract/persistence/share-cap failures back off or fail closed without blocking Earth heartbeat/Wake/Care/Delivery; P6.1 malformed/undelivered/colliding feedback fails closed without delivery or external side effects; P6.2 provider/contract failure leaves reply unreviewed, applies bounded backoff and cannot block heartbeat or existing Care delivery; P6.3/P6.4 replay/collision/corrupt-state paths fail closed without creating external side effects | COVERED |
...
| OH-P6 | User feedback affects future strategy without direct overwrite | P4.5 protects Feedback→Preference→review→Soul bounds; P6.1 `relationship-feedback.test.ts` protects delivered-message binding, explicit-vs-ambiguous signal semantics, fixed strategy mappings, idempotency and no direct Soul/delivery/Android mutation; P6.2 `relationship-reply-review*.test.ts` protects observed/user-declared/inferred separation, strict proposal-only model output, no direct Preference/Soul writes, sparse budgets and Worker failure isolation; P6.3 `relationship-reply-proposal-review.test.ts` protects explicit confirm/correct/dismiss, inferred-proposal immutability, fixed P4.5 mappings, restart/dedupe and no direct Soul rewrite; P6.4 relationship feedback review/revocation tests protect bounded audit linkage, active-evidence-only Preference rebuild, review invalidation, correction ordering, restart/dedupe/collision, generic revocation validation and no direct Soul mutation; acceptance recorded in `OH_P6_ACCEPTANCE.md` | COVERED |
...
## OH-P6 evidence split

P6.1 automated:

- relationship signals bind one exact existing delivered proactive candidate;
- feedback timestamps cannot precede `deliveredAt` or occur after capture time;
- explicit `like`/`dislike`/`accept_suggestion`/`reject_suggestion` actions persist as `EARTH/user_declared/RELATIONSHIP`;
- ambiguous `reply`/`ignore` persist as `EARTH/observed/RELATIONSHIP` and do not automatically create Preference evidence;
- explicit product signals map to fixed strategy interests and fixed P4.5 feedback signals; callers cannot inject arbitrary interest keys or learning strength;
- exact `signalKey` replay is idempotent and conflicting reuse fails closed;
- the referenced proactive message is not rewritten by feedback capture;
- capture adds no model call, notification, Android action, observation mutation or direct Soul write;
- restart preserves relationship-signal records and dedupe behavior.

Test: `test/relationship-feedback.test.ts`.
Implementation note: `docs/P6_RELATIONSHIP_FEEDBACK_V01.md`.

P6.2 automated:

- reply content binds only to an existing P6.1 `reply` signal and is persisted separately as `EARTH/user_declared/RELATIONSHIP`;
- one reply signal has one canonical bounded text record with deterministic digest;
- model review input contains only the exact reply and delivered proactive-message context;
- strict output is `ignore | propose_feedback` with one fixed proposal class and bounded public summary;
- arbitrary interest/strength/Preference/Soul/send/notify/action/hidden-reasoning fields fail closed;
- proposals persist as `EARTH/inferred/RELATIONSHIP` and never call P4.5 automatically;
- six-hour success cooldown, one-hour failure backoff, three provider attempts per UTC day and a processing lease bound cognition cost;
- retry/restart cannot duplicate the same proposal basis;
- review is optional inside the existing single-owner Life Loop and provider failure cannot block heartbeat or existing Care delivery.

Tests: `test/relationship-reply-review.test.ts`, `test/relationship-reply-review-worker.test.ts`.
Implementation note: `docs/P6_REPLY_INTERPRETATION_V01.md`.

P6.3 automated:

- a P6.2 inferred proposal remains immutable and cannot enter P4.5 without a separate explicit user review;
- user review persists as `EARTH/user_declared/RELATIONSHIP` with stable `reviewKey`;
- `confirm` and `correct` can only resolve to the four fixed directional strategy classes;
- non-directional `correction` cannot be confirmed directly into learning;
- `dismiss` is terminal and creates no P4.5 feedback/Preference evidence;
- original inferred proposal provenance/basis/summary remains unchanged;
- review persistence precedes deterministic P4.5 reconciliation and replay after restart cannot duplicate feedback/evidence;
- conflicting `reviewKey` reuse or second terminal review fails closed;
- no model call, notification, Android/external action or direct Soul rewrite is introduced.

Test: `test/relationship-reply-proposal-review.test.ts`.
Implementation note: `docs/P6_REPLY_PROPOSAL_CONFIRMATION_V01.md`.

P6.4 automated:

- bounded review joins exact P4.5 Earth feedback to exact active or archived AI World evidence;
- separate Earth/user-declared and AI World/inferred revocation records preserve provenance boundaries;
- stable `revocationKey` replay is idempotent and conflicting reuse/second revoke fails closed;
- original feedback and original evidence remain auditable after revoke;
- revoked evidence is removed from the active Preference basis without fabricating opposite evidence;
- remaining Preference score/evidenceCount/evidenceIds are rebuilt only from active evidence;
- active-basis change clears old `lastReviewedAt` so Soul cannot reuse a pre-revocation review;
- if no active evidence remains, temporary Preference is removed;
- correction revokes the exact old evidence before one new bounded P4.5 correction record is created;
- crash/restart between Earth revocation persistence and AI World reconciliation is recoverable without duplication;
- existing Soul tendency/change state is not directly mutated by revoke/correct;
- historical Soul basis references remain valid through the revoked-evidence audit archive;
- generic AI World validation rejects forged revocation boundary, missing/unrelated targets, duplicate identities and orphaned archive entries;
- Earth review validation rejects corrupt/duplicate Earth revocation records.

Tests: `test/relationship-feedback-review.test.ts`, `test/relationship-feedback-revocation.test.ts`, P6.4 Soul-boundary and revocation-validation regression tests.
Acceptance: `docs/OH_P6_ACCEPTANCE.md`.

P6 final Phase Review confirms that silence/absence is never silently treated as dislike, model proposals cannot self-promote into learning, P4.5 remains the only Earth→AI World feedback Bridge, and Soul can change only through the existing explicit Preference review + bounded P4.3 gate. Runtime/domain learned-evidence review/revoke/correct is complete; product UI and broader delete/pause controls remain OH-41 follow-up work.
...
A bug fix should identify which design requirement was violated. The regression test should reference the requirement in its test name or nearby comment when practical.
