...
export class JsonStore {
  ...
  getLifeContext(observedAt = now()): LifeContext {
    ...
    const activeObservations = activePhoneDeviceId ? data.observations.filter((item) => item.deviceId === activePhoneDeviceId) : data.phoneDeviceRegistrations.length === 0 ? data.observations : data.observations.filter((item) => !item.deviceId);
  ...
  async evaluateWakeEvents(observedAt = now()): Promise<WakeEvent[]> {
    const current = deriveLifeState(this.data.activePhoneDeviceId ? this.data.observations.filter((item) => item.deviceId === this.data.activePhoneDeviceId) : this.data.phoneDeviceRegistrations.length === 0 ? this.data.observations : this.data.observations.filter((item) => !item.deviceId), observedAt);
