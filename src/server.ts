...
export function createOurHomeServer(store: JsonStore): McpServer {
  ...
  server.registerTool(
    "home.write_diary",
    {
      ...
      description: "Create a structured diary entry. New callers should provide an explicit world/provenance boundary; legacy calls without both are quarantined as FICTION/authored rather than guessed into Earth.",
      inputSchema: {
        title: z.string().trim().min(1).max(200),
  ...
  server.registerTool(
    "home.create_action",
    {
      ...
      description: "Create a new Our Home action item. New callers should provide an explicit world/provenance boundary; legacy calls without both are quarantined as FICTION/authored. This does not start an external task by itself.",
      inputSchema: {
        title: z.string().trim().min(1).max(200),
