...
test("unauthorized registration cannot persist a push address", async () => {
  const store = await freshStore();
  await assert.rejects(registerPhone(store, "secret", "Bearer wrong", { deviceId: "android-main", pushToken: "sensitive" }), /Unauthorized/);
